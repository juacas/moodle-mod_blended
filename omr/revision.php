...
// Get the params ---------------------------------------------------------------- 
    ...
    add_to_log($course->id, "blended", "revision", "revision.php?a=$blended->id", "$blended->id");
...
// Capabilities ----------------------------------------------------- 
    ...
    $url =  new moodle_url('/mod/blended/revision.php',array('id'=>$id,'a'=>$a));
    $PAGE->set_url($url);
...
//                                          array('name' => $strrevisionpage,'link'=>null, 'type'=>'misc')));
//     print_header("$course->shortname: $blended->name: $strrevisionpage", "$course->shortname",$navigation, 
//                   "", "", true, update_module_button($cm->id, $course->id, $blended->name, $strrevisionpage), 
//                   navmenu($course, $cm));
...
// Print the main part of the page ---------------------------------- 
    ...
    echo $OUTPUT->heading(format_string(get_string('revision', 'blended')));
    echo $OUTPUT->box(format_text(get_string('revisionpagedesc', 'blended')), 'generalbox', 'intro');
    echo $OUTPUT->spacer(array('height'=>20));
    
    /**
     * Review current User results
     */
    review_results($blended,$USER);
    
    echo "<BR><BR><center>";
//     helpbutton($page='revision', get_string('pagehelp','blended'), $module='blended', $image=true, $linktext=true, $text='', $return=false,$imagetext='');
